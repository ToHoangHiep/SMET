...
class ProjectProgressMobile extends StatelessWidget {
  final Widget statsGrid;
  final Widget progressList;
  final String userName;
  final VoidCallback onLogout;

  const ProjectProgressMobile({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Tiến độ'), backgroundColor: const Color(0xFF137FEC), foregroundColor: Colors.white, leading: IconButton(icon: const Icon(Icons.menu), onPressed: () => _showDrawer(context))),
      drawer: _buildDrawer(context),
      body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [statsGrid, const SizedBox(height: 16), progressList])),
    );
  }
  ...
  Widget _buildDrawer(BuildContext context) => Drawer(
    child: ListView(padding: EdgeInsets.zero, children: [
      ...
      ListTile(leading: const Icon(Icons.trending_up), title: const Text('Tiến độ'), selected: true, onTap: () => Navigator.pop(context)),
